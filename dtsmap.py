...
class dtsmap(object):
	...
	def run(self,Test=False, Verbose=False, UseGtlike=False):
		...
		for raStep in ra_range:
			for decStep in dec_range:
				...
				process = 'bsub -o ' + logfile + ' -J ' + jobName + ' -q long -R rhel60 -g ' + JobsDirectory + ' "' + command + '"'
...
#		AddCandidateSource(self.xref, self.yref, xmlModelWithPutativeSource)
		...
		try:
			...
			like.fit(verbosity=0,covar=True,tol=1e-5,optimizer='MINUIT', optObject=None)
